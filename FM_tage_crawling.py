...
STATE_PATH = APP_DIR / "runtime" / "state" / "tage_shopee_state.json"
...
TAGE_COUNTRYLIST = {
    ...
    "Philippines": "https://seller.shopee.kr/portal/sale/order/pre-declare/generate?cnsc_shop_id=1010596078"
}
...
def run(country_input: list[str]) -> None:
    ...
    kst_hm = datetime.now().strftime("%H%M")
    output_dir = APP_DIR / f"TAGE_FM_{kst}"
    ...
    with sync_playwright() as p:
        ...
        try:
            for country in country_input:
                ...
                saved = download_pdf(
                    popup,
                    save_path=str(output_dir / f"TAGE_FM_{country}_{kst}_{kst_hm}.pdf"),
                )
