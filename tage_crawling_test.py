...
json_path = APP_DIR / "tage_shopee_state.json"
...
def resolve_json_path() -> Path:
    if json_path.exists():
        return json_path
    raise FileNotFoundError(f"테스트용 state 파일이 없습니다: {json_path}")
...
def create_pickup_and_download_pdf(
    ...
    for i in range(label_cnt):
        ...
        while True:
            ...
            # Arrange Shipment Progress 팝업 안에서 진행 상태가 충분히 끝났는지 브라우저 쪽에서 계속 감시
...
def run(country_input: list[str], progress_callback: Callable | None = None) -> dict:
    ...
    output_dir = APP_DIR / f"TAGE_TEST_{kst}"
    totals = {
        "shopee_total_pages": 0,
        "downloaded_pages": 0,
        "merged_downloaded_pages": 0,
        ...
        try:
            for country in country_input:
                ...
                pdf_to_text(
                    str(output_dir),
                    country,
                    f"{APP_DIR}/TAGE_TEST_{country}_{kst}.xlsx",
                )
        finally:
            context.close()
            browser.close()
...
if __name__ == "__main__":
    selected_countries = sys.argv[1:] or DEFAULT_TEST_COUNTRIES
